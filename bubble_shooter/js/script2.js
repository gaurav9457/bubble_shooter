...
function createShooterBubble(context) {
    ...
    //console.log("shooterX ",shooterX,"shootery ",shooterY);
...
canvasFrame.addEventListener('click', function(event) {
	...
	const horizontal = 2; 
    if (Math.abs(event.offsetY - 630) <= horizontal) {
        console.log("User clicked at the horizontal level of the shooter bubble");
	...
	//check they are adjacent and  same color 
    function AdjacentAndSameColor(bubble1, bubble2) {
       ...
       // console.log(shooterBubble.color, "Clicked on color shooterbubble");
        ...
        if (clickedBubble.color === shooterBubble.color) {
            ...
            const sameColorAdjacentBubbles = bubbles.filter(bubble => {        
                return AdjacentAndSameColor(clickedBubble, bubble);
            });
